...
export async function initializeDb() {
  ...
  try {
    ...
    await sql`CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, username VARCHAR(50) UNIQUE NOT NULL, name VARCHAR(100) NOT NULL, password_hash TEXT NOT NULL, role VARCHAR(20) DEFAULT 'user', is_active BOOLEAN DEFAULT TRUE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`;
    await sql`CREATE TABLE IF NOT EXISTS activity_logs (id SERIAL PRIMARY KEY, user_id INTEGER, user_name VARCHAR(100), action VARCHAR(50) NOT NULL, target_type VARCHAR(50), target_id INTEGER, detail TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`;
...
export async function getUserByUsername(username) {
  if (!sql) return null;
  const rows = await sql`SELECT * FROM users WHERE username = ${n(username)} AND is_active = TRUE`;
  return rows[0] || null;
}

export async function getUserById(id) {
  if (!sql) return null;
  const rows = await sql`SELECT id, username, name, role, is_active, created_at FROM users WHERE id = ${n(id)}`;
  return rows[0] || null;
}

export async function getAllUsers() {
  if (!sql) return [];
  return await sql`SELECT id, username, name, role, is_active, created_at FROM users ORDER BY created_at ASC`;
}
...
export async function createUser(data) {
  ...
  const rows = await sql`
    ...
    VALUES (${n(data.username)}, ${n(data.name)}, ${n(data.password_hash)}, ${n(data.role || 'user')})
    RETURNING id
  `;
  return { id: rows[0].id };
}
...
export async function deleteUser(id) {
  if (!sql) return;
  await sql`DELETE FROM users WHERE id = ${n(id)}`;
}

export async function updateUserPassword(id, passwordHash) {
  if (!sql) return;
  await sql`UPDATE users SET password_hash = ${n(passwordHash)} WHERE id = ${n(id)}`;
}
...
export async function createActivityLog(data) {
  if (!sql) return;
  await sql`
    INSERT INTO activity_logs (user_id, user_name, action, target_type, target_id, detail)
    VALUES (${n(data.user_id)}, ${n(data.user_name)}, ${n(data.action)}, ${n(data.target_type)}, ${n(data.target_id)}, ${n(data.detail)})
  `;
}
